...
public class DistanceDispatchRule extends AbstractIDispatchRule {
    ...
    public List<ServeProviderDispatchDTO> doFilter(List<ServeProviderDispatchDTO> originServeProviderDTOS) {
        ...
        originServeProviderDTOS = originServeProviderDTOS.stream().sorted(Comparator.comparing(ServeProviderDispatchDTO::getAcceptanceDistance)).collect(Collectors.toList());
        // 3.遍历优先级最高一批数据
        ServeProviderDispatchDTO first = CollUtils.getFirst(originServeProviderDTOS);

        //获取相同级别的
        return originServeProviderDTOS.stream()
                .filter(origin -> Comparator.comparing(ServeProviderDispatchDTO::getAcceptanceDistance).compare(origin, first) == 0)
