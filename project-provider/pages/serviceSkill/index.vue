  ...
  getServiceSkillAll()
    .then((res) => {
      if (res.code == 200) {
        ...
        rightItem.data = serviceSkill.data[0].serveSkillItemDTOList;
      }
    })
    .catch((err) => {
      uni.showToast({
...
const handleSelect = (active) => {
  ...
  (serviceSkill.data[activeIndex.value].serveSkillItemDTOList =
    serviceSkill.data[activeIndex.value].serveSkillItemDTOList.map(
      (item) => {
...
const handleSubmit = () => {
  ...
  serviceSkill.data.forEach((item) => {
    item.serveSkillItemDTOList.forEach((item1) => {
      if (item1.isSelected) {
        selectedList.push({
