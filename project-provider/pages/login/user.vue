...
<template>
  <view>
    <image src="@/static/new/img_logo@2x.jpg" class="logo"></image>
    <view class="loginBox">
      <!-- 登录表单 -->
      <view class="loginMain">
        <uni-forms ref="customForm" :rules="customRules" :modelValue="fromInfo">
          ...
          <uni-forms-item name="veriryCode" class="pwdBox">
            <uni-easyinput
              :clearable="false"
              class="item vcode-input"
              v-model="fromInfo.veriryCode"
              placeholder="请输入验证码"
              style="padding-right: 140rpx;"
            />
            <view class="clearIcon" @tap.stop="handlePwd">获取验证码</view>
            <!-- 添加一个透明的可点击按钮，增加可点击面积 -->
            <button
...
const handlePwd = async () => {
  try {
    console.log('当前 API 请求地址:', import.meta.env.VITE_API_URL || '未设置');
    const reg = /^1[3-9]\d{9}$/;
    ...
    uni.hideLoading();
    uni.showToast({
      title: error.msg || error.message || '获取验证码失败',
      duration: 1500,
      icon: 'none',
    });
  }
